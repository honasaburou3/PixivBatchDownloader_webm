...
Внимание: эта функция очищает download record загрузчика, а не download history браузера.`,
  ...
  _下载插画漫画时转换为WebP: [
    `下载插画、漫画时转换为 WebP 图片（长边 2048px）`,
    `下載插畫、漫畫時轉換為 WebP 圖片（長邊 2048px）`,
    `Convert illustrations and manga to WebP (max long side: 2048px)`,
    `イラスト・マンガを WebP に変換（長辺2048px）`,
    `일러스트와 만화를 WebP로 변환 (긴 변 2048px)`,
    `Конвертировать иллюстрации и мангу в WebP (длинная сторона: 2048 пикс.)`,
  ],
  _质量: ['质量', '品質', 'Quality', '品質', '품질', 'Качество'],
  _建议95及以下: [
    '（建议不超过 95）',
    '（建議不超過 95）',
    '(recommended: 95 or lower)',
    '（推奨95まで）',
    '(권장: 95 이하)',
    '(рекомендуется не выше 95)',
  ],
  _转换后文件更大时保存原文件: [
    '转换后的文件大于原文件时，保存原文件',
    '轉換後的檔案大於原檔案時，儲存原檔案',
    'Save the original file when the converted file is larger',
    '変換後のファイルが元ファイルより大きい場合は元ファイルを保存',
    '변환 후 파일이 원본보다 크면 원본 파일 저장',
    'Сохранять исходный файл, если преобразованный файл больше',
  ],
  _图片尺寸: [
    '图片尺寸',
    '圖片尺寸',
